...
import II_share_functions as sf

from share_settings import Settings
from II_backtest import Strategy, Portfolio
...
if __name__ == "__main__":
    # Obtain daily bars of stock (ETF that generally 
    # follows the S&P500) from Quandl (requires 'pip install Quandl'
    # on the command line)
    symbol = 'ADM'
    
    bars = pdr.DataReader(symbol,'google',s.st_date,s.ed_date)

    # Create a set of random forecasting signals for stock
    rfs = RandomForecastingStrategy(symbol, bars)
    signals = rfs.generate_signals()

    # Create a portfolio of stock
